...
#ПРИМЕР ПОИСКА НАИЛУЧШЕГО СЦЕНАРИЯ ОБРАБОТКИ И ТЕМАТИЧЕСКОГО МОДЕЛИРОВАНИЯ, МЕТРИКИ И ОПТИММИЗАЦИЯ НА ОСНОВЕ ARTM
...
#предобработали корпус всеми предложенными стратегиями (последовательностями функций)
    ...
    for i in range(len(X_trains)):
        ...
        for num_topics in [3, 6, 9, 12]:
            ...
            bv = artm.BatchVectorizer(data_format='bow_n_wd', 
                                      n_wd=X_train_vect.toarray(), 
                                      vocabulary=list(vectorizer.vocabulary_.keys()))

            artm_model = ARTM(num_topics = num_topics, 
                       dictionary=bv.dictionary, 
                       scores=[artm.TopTokensScore(name='TopTokensScore', 
                                                   num_tokens=8)])


            artm_model.fit_offline(bv)
            
            #предсказали соотвествие элементов корпуса кластерам
            
            
            prediction = np.matrix(artm_model.transform(X_train_vect)).argmax(axis=1)
            ...
            for topic, comp in enumerate(artm_model.components_):
    ...
    artm_model = ARTM(num_topics = results['num_topics'], 
                       dictionary=bv.dictionary, 
                       scores=[artm.TopTokensScore(name='TopTokensScore', 
                                                   num_tokens=8)])
